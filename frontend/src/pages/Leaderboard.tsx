...
const Leaderboard = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [location.pathname]); // Added location.pathname to refresh on navigation
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-primary">{streak?.longestStreak || 0} days</p>
          ...
          >
            <Card className="glass-card p-6">
              ...
              <div className="grid grid-cols-7 gap-2">
                {streak.streakHistory.slice(0, 7).map((day: any, index: number) => (
                  <motion.div
                    key={index}
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                  ...
                  >
                    <div className={`aspect-square rounded-lg flex items-center justify-center ${day.completed
        ...
        >
          <div className="grid grid-cols-3 gap-4 max-w-4xl mx-auto">
            ...
            {secondPlace && (
              ...
              >
                <Card className="glass-card p-4 text-center hover-lift mt-8">
                  <div className="text-4xl mb-2">{secondPlace.badge || '🥈'}</div>
                  <Avatar className="h-16 w-16 mx-auto mb-2 ring-4 ring-muted">
                    <AvatarImage src={secondPlace.avatar} />
                    <AvatarFallback>{secondPlace.name?.[0]}</AvatarFallback>
                  </Avatar>
                  <p className="font-bold">{secondPlace.name}</p>
                  <p className="text-2xl font-bold text-muted-foreground my-2">{secondPlace.score}</p>
                  <div className="flex items-center justify-center gap-1 text-sm text-orange-500">
                    <Flame className="h-4 w-4" />
                    {secondPlace.streak} days
                  </div>
                </Card>
              </motion.div>
            )}
            ...
            {firstPlace && (
              ...
              >
                <Card className="glass-card p-4 text-center hover-lift border-2 border-primary glow-primary">
                  <div className="text-5xl mb-2">{firstPlace.badge || '🥇'}</div>
                  <Avatar className="h-20 w-20 mx-auto mb-2 ring-4 ring-primary">
                    <AvatarImage src={firstPlace.avatar} />
                    <AvatarFallback>{firstPlace.name?.[0]}</AvatarFallback>
                  </Avatar>
                  <p className="font-bold text-lg">{firstPlace.name}</p>
                  <p className="text-3xl font-bold text-primary my-2">{firstPlace.score}</p>
                  <div className="flex items-center justify-center gap-1 text-sm text-orange-500">
                    <Flame className="h-4 w-4" />
                    {firstPlace.streak} days
                  </div>
                </Card>
              </motion.div>
            )}
            ...
            {thirdPlace && (
              ...
              >
                <Card className="glass-card p-4 text-center hover-lift mt-8">
                  <div className="text-4xl mb-2">{thirdPlace.badge || '🥉'}</div>
                  <Avatar className="h-16 w-16 mx-auto mb-2 ring-4 ring-muted">
                    <AvatarImage src={thirdPlace.avatar} />
                    <AvatarFallback>{thirdPlace.name?.[0]}</AvatarFallback>
                  </Avatar>
                  <p className="font-bold">{thirdPlace.name}</p>
                  <p className="text-2xl font-bold text-muted-foreground my-2">{thirdPlace.score}</p>
                  <div className="flex items-center justify-center gap-1 text-sm text-orange-500">
                    <Flame className="h-4 w-4" />
                    {thirdPlace.streak} days
                  </div>
                </Card>
              </motion.div>
            )}
            ...
            <div className="space-y-3">
              {leaderboard.map((user, index) => (
                <motion.div
                  ...
                  className={`flex items-center gap-4 p-4 rounded-lg glass hover-lift ${user.isCurrentUser ? 'ring-2 ring-primary bg-primary/5' : ''
                    }`}
                >
